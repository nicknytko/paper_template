...
\renewcommand{\comment}[2]{{\color{#1} #2}}
\newcommand{\nn}[1]{\comment{teal}{NN\@: #1}}
...
\newcommand{\mask}[1]{\maskOp\left({#1} \right)}
\newcommand{\nnz}[1]{\nnzOp\left({#1} \right)}
\newcommand{\gradfn}[2]{\nabla_{{#1}}\left({#2}\right)} % grad with respect to #1 of #2
\newcommand{\jac}[2]{J_{{#1}}\left({#2}\right)} % jacobian with respect to #1 of #2
...
\vskip 0.2in
\begin{center}
\centerline{\includegraphics[width=\columnwidth]{sine.pdf}}
\caption{A sine wave.}
\label{fig:sine}
\end{center}
\vskip -0.2in
